# 🐍 Python Programming MOOC 2026

![Python](https://img.shields.io/badge/Python-3.x-blue?style=flat&logo=python&logoColor=white)
![University](https://img.shields.io/badge/University-Helsinki-blue?style=flat)
![Course](https://img.shields.io/badge/Course-MOOC%202026-orange?style=flat)
![Progress](https://img.shields.io/badge/Progress-0%25-red?style=flat)
![Status](https://img.shields.io/badge/Status-In%20Progress-yellow?style=flat)

> 📚 Personal learning journal for the **Python Programming MOOC 2026** offered by the University of Helsinki. This repository tracks my progress, exercises, and notes across all 14 parts of the course.

**Course link:** [programming-26.mooc.fi](https://programming-26.mooc.fi/)

---

## About the Course

The Python Programming MOOC 2026 is a free online course from the **Department of Computer Science at the University of Helsinki**. It is divided into two tracks:

| Track | Parts | Credits |
|---|---|---|
| Introduction to Programming (BSCS1001) | Parts 1 – 7 | 5 ECTS |
| Advanced Course in Programming (BSCS1002) | Parts 8 – 14 | 5 ECTS |

The course runs throughout the year 2026 and covers Python from the very basics up to advanced programming concepts.

Note: Some of the exercise text may be in my native language, Brazilian Portuguese.

---

## 🗂️ Repository Structure

```
📦 python-mooc-2026/
├── 📁 part-01/
│   ├── exercises/
│   └── notes.md
├── 📁 part-02/
│   ├── exercises/
│   └── notes.md
├── 📁 part-03/
│   ├── exercises/
│   └── notes.md
├── 📁 part-04/
│   ├── exercises/
│   └── notes.md
├── 📁 part-05/
│   ├── exercises/
│   └── notes.md

...

── 📄 README.md
```

---

## 📊 Progress Tracker

### Introduction to Programming

| Part | Topic | Status | Exercises | Completed |
|------|-------|--------|-----------|-----------|
| Part 1 | Getting started | 🔄 In Progress | 5 / 33 | — |
| Part 2 | Variables and conditionals | ⏳ Not started | 0 / 23 | — |
| Part 3 | Loops | ⏳ Not started | 0 / 35 | — |
| Part 4 | Functions | ⏳ Not started | 0 / 39 | — |
| Part 5 | Lists | ⏳ Not started | 0 / 28 | — |
| Part 6 | Files and error handling | ⏳ Not started | 0 / 20 | — |
| Part 7 | Data structures | ⏳ Not started | 0 / 20 | — |

### Advanced Course in Programming

| Part | Topic | Status | Exercises | Completed |
|------|-------|--------|-----------|-----------|
| Part 8  | Classes and objects | ⏳ Not started | 0 / 40 | — |
| Part 9  | Objects and references | ⏳ Not started | 0 / 40 | — |
| Part 10 | Class hierarchies | ⏳ Not started | 0 / 40 | — |
| Part 11 | List comprehensions | ⏳ Not started | 0 / 40 | — |
| Part 12 | Functional programming | ⏳ Not started | 0 / 40 | — |
| Part 13 | Pygame and GUI | ⏳ Not started | 0 / 40 | — |
| Part 14 | Final project | ⏳ Not started | — | — |

**Legend:** ✅ Completed &nbsp;|&nbsp; 🔄 In Progress &nbsp;|&nbsp; ⏳ Not Started

---

## 🧠 Key Learnings

> This section is updated as each part is completed.

### Part 1 — Getting started
_Coming soon..._

### Part 2 — Variables and conditionals
_Coming soon..._

<!-- Continue updating as you complete each part -->

---

## 💡 Reflections & Challenges

> Personal notes on what was hard, what surprised me, and how I overcame obstacles.

| Part | Challenge faced | How I solved it |
|------|----------------|-----------------|
| — | — | — |

---

## 🚀 How to Run the Exercises

1. **Clone this repository:**
   ```bash
   git clone https://github.com/YOUR-USERNAME/python-mooc-2026.git
   cd python-mooc-2026
   ```

2. **Make sure Python 3 is installed:**
   ```bash
   python --version
   ```

3. **Run any exercise file:**
   ```bash
   python part-01/exercises/exercise_name.py
   ```

---

## 🔗 Useful Links

- 📘 [Course Website](https://programming-26.mooc.fi/)
- 💬 [Course Discord](https://study.cs.helsinki.fi/discord/join/ohjelmoinnin_mooc)
- 🐍 [Official Python Docs](https://docs.python.org/3/)
- 🎓 [University of Helsinki](https://www.helsinki.fi/en)

---

## 📅 Timeline

- 🗓️ **Course start:** January 12, 2026
- 🗓️ **Course end:** December 31, 2026
- 🗓️ **Advanced exam:** December 16, 2026

---

## 👤 About Me

Hi! I'm **Renaldo**, currently learning Python through this MOOC. Feel free to follow my journey or reach out! I actually started the course last year but didn't finish it. This time, I'm seeing it through to the end!

- GitHub: [@renaldofreire](https://github.com/renaldofreire)
- LinkedIn: [Renaldo Freire](https://www.linkedin.com/in/renaldofreire)

---

<p align="center">
  Made with ❤️ while learning Python · University of Helsinki MOOC 2026
</p>
